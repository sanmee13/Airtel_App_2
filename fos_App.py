...
def load_excel_database():
    file_path = "fos_data.xlsx"
    if not os.path.exists(file_path):
        st.error(f"❌ Critical Error: '{file_path}' file not found. Please place it in the application folder.")
        return pd.DataFrame()
    try:
...
# ── VIEW SCREEN 1: SEARCH DIRECTORY ───────────────────────────────────
if st.session_state.current_view == "search" and not main_df.empty:
    # Compute metrics dynamically from your unique raw rows inside the Excel sheet!
    ...
    # Create searchable directory lists
...
# ── VIEW SCREEN 2: DYNAMIC ANALYTICS DASHBOARD ────────────────────────
    ...
    if fos_df.empty:
        ...
        # Extract metadata metrics safely from first row match
        f_row = fos_df.iloc[0]
        meta = {"fos": f_row['FOS_Number'], "tm": f_row['TM_Name'], "zm": f_row['Zone_Manager'], "zn": f_row['Zone_Name']}
        
        # Build optimized metrics objects for every localized row matching old JS variables 
        outlets = []
        ...
        # Split categories dynamically using analytical filters 
        ...
        # Global Top-Level Dashboard Math Calculations
        ...
        # Prioritize Priority Matrices Stack Generation (Max limit 10 elements)
        tasks = []
        used_rets = set()
        ...
        # Rendering Style Directives Maps
        TAG_STYLES = {
        ...
        # Alerts System Markup processing 
        alert_items = []
        ...
        # Main Performance Matrix KPI Grid block rendering
        ...
        # Plan List Loop markup string compiling
        ...
        # Native Action System Interface Go Back Button Execution
